...
\begin{remark}
  Като частен случай на \Proposition{unrestricted-grammar:context-general-step} получаваме следното твърдение:
  \begin{prooftree}
...
\end{proposition}
\begin{proof}
  Нека $A \yield{\leq \ell+1} \alpha$.
  Имаме два случая.
  \begin{itemize}
  \item
    $A \yield{\leq \ell} \alpha$. Щом $A \in V$, а $\alpha \in \Sigma^\star$, то е ясно, че $\ell > 0$.
    Тогава имаме, че:
    \begin{prooftree}
      ...
      \QuaternaryInfC{$A \yield{\leq\ell} \underbrace{\alpha_1\beta_1\cdots\alpha_n\beta_n\alpha_{n+1}}_{\alpha}$}
    \end{prooftree}
    Понеже $B_i \yield{\leq\ell} \beta_i$, то от \IndHyp имаме, че $\beta_i \in \L^{\ell}_G(B_i)$.
    Тогава $\alpha \in \L^{\ell+1}_G(A)$.
  \item
    Ако $A \yield{\ell+1} \alpha$, то имаме, че:
    \begin{prooftree}
      \AxiomC{$A \to_G \alpha_1B_1\cdots\alpha_n B_n\alpha_{n+1}$}
      ...
      \LeftLabel{\scriptsize{($\ell = \max\{\ell_1,\dots,\ell_n\})$}}
      \RightLabel{\scriptsize{(1)}}
      \QuaternaryInfC{$A \yield{\ell+1} \underbrace{\alpha_1\beta_1\cdots\alpha_n\beta_n\alpha_{n+1}}_{\alpha}$}
    \end{prooftree}
    Понеже $B_i \yield{\leq \ell} \beta_i$, то от \IndHyp имаме, че $\beta_i \in \L^\ell_G(B_i)$.
    Тогава $\alpha \in \L^{\ell+1}_G(A)$.
  \end{itemize}
  Нека $\alpha \in \L^{\ell+1}_G(A)$. Тогава имаме, че $A \to_G \alpha_1B_1\cdots\alpha_nB_n\alpha_{n+1}$
  и $\beta_i \in \L^\ell_G(B_i)$ и $\alpha = \alpha_1\beta_1\cdots\alpha_n\beta_n\alpha_{n+1}$.
