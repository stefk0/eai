...
\begin{example}
  \begin{align*}
    & S \to \texttt{if } S \texttt{ then } S \texttt{ else }S\quad |\quad \texttt{ if }S \texttt{ then }S\quad |\quad V\\
    & V \to x\quad |\quad y\quad |\quad z
  \end{align*}

  Ние искаме следната граматика:
  \begin{align*}
    & S \to M\quad |\quad U\\
    & M \to \texttt{if } S \texttt{ then } M \texttt{ else }M\quad |\quad X\\
    & U \to \texttt{if } S \texttt{ then } S\quad |\quad \texttt{if } S \texttt{ then } M \texttt{ else }U
  \end{align*}
\end{example}
