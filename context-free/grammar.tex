...
\end{proposition}
\begin{hint}
  За посоката $(\leftarrow)$, нека $\alpha = \alpha_1 A \alpha_2$, $A \to_G \gamma$ и $\beta = \alpha_1 \gamma \alpha_2$. Тогава
  \begin{prooftree}
    \AxiomC{$\alpha_1 A \alpha_2 \derive{0} \alpha_1 A \alpha_2$}
    \AxiomC{$A \derive{1} \gamma$}
    \RightLabel{\scriptsize{правило (3)}}
    \BinaryInfC{$\underbrace{\alpha_1 A \alpha_2}_{\alpha} \derive{1} \underbrace{\alpha_1\gamma\alpha_2}_{\beta}$}
  \end{prooftree}
  За посоката $(\rightarrow)$, индукция по $\abs{\alpha}$.
  Ако $\abs{\alpha} = 1$, то е ясно. Нека $\abs{\alpha} > 1$. Тогава имаме $\ell' + \ell'' = 1$
  \begin{prooftree}
    \AxiomC{$\alpha' \derive{\ell'} \beta'$}
    \AxiomC{$\alpha'' \derive{\ell''} \beta''$}
    \BinaryInfC{$\underbrace{\alpha'\alpha''}_{\alpha} \derive{\ell'+\ell''} \underbrace{\beta'\beta''}_{\beta}$}
  \end{prooftree}
  Без ограничение на общността, нека $\ell' = 1$ и $\ell'' = 0$. Понеже $\abs{\alpha'} < \abs{\alpha}$, можем да приложим \IndHyp
  за $\alpha'$. Получаваме, че $\alpha' = \alpha'_1A\alpha'_2$, $A \to_G \gamma$ и $\beta' = \alpha'_1 \gamma \alpha'_2$.
  Ясно е, че тогава $\alpha_1 = \alpha'_1$ и $\alpha_2 = \alpha'_2 \alpha''$.
\end{hint}
