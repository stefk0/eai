...
Ясно е, че $\L(G) = \L_G(S)$.
Нека да дефинираме и \emph{апроксимациите} $\L^\ell_G(A)$ на езика $\L_G(A)$ по следния начин:
...
\begin{proposition}\label{pr:grammar:yield-approximation}
  ...
  \begin{align*}
    & \L^0_G(A) = \emptyset\\
    & \L^{\ell+1}_G(A) = \bigcup\{\{\alpha_0\}\cdot \L^\ell_G(A_1) \cdots \{\alpha_{n-1}\} \cdot \L^\ell_G(A_n) \cdot \{\alpha_{n}\} \mid & A \to_G \alpha_0A_1\cdots\alpha_{n-1}A_n\alpha_{n} \text{ е правило }\\
    & & \text{в $G$, където }\alpha_i \in \Sigma^\star\text{ за }i\leq n\}.
  \end{align*}
\end{proposition}
\begin{proof}
  Доказателството протича с индукция по $\ell$.
  Твърдението очевидно е изпълнено за $\ell = 0$.
  За индукционната стъпка, първо ще докажем включването $\subseteq$.Нека сега да разгледаме дума $\alpha \in \L^{\ell+1}_G(A)$, т.е. $A \yield{\leq \ell+1} \alpha$.
  Тогава имаме следния извод:
  \begin{prooftree}
    \AxiomC{$A \to_G \alpha_0B_1\cdots\alpha_{n-1} B_n\alpha_{n}$}
    \AxiomC{$B_1 \yield{\leq \ell} \beta_1$}
    \AxiomC{$\cdots$}
    \AxiomC{$B_n \yield{\leq \ell} \beta_n$}
    \QuaternaryInfC{$A \yield{\leq\ell+1} \underbrace{\alpha_0\beta_1\cdots\alpha_{n-1}\beta_n\alpha_{n}}_{\alpha}$}
  \end{prooftree}
  Понеже $B_i \yield{\leq \ell} \beta_i$, то от \IndHyp имаме, че $\beta_i \in \L^\ell_G(B_i)$ за всяко $i = 1,2,\dots,n$.
  Тогава 
  ...
  Сега преминаваме към доказателството на включването $\supseteq$. Нека вземем дума $\alpha$ принадлежаща на дясното множество. Това означава, че можем да представим $\alpha$ като $\alpha = \alpha_0\beta_1\cdots\alpha_{n-1}\beta_n\alpha_{n}$, където $A \to_G \alpha_0B_1\cdots\alpha_{n-1}B_n\alpha_{n}$,
  $\beta_i \in \L^\ell_G(B_i)$ и $\alpha = \alpha_0\beta_1\cdots\alpha_{n-1}\beta_n\alpha_{n}$.
