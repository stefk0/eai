...
\begin{problem}
  ...
  \item
    ...
    $L = \{\omega \in \{a,b\}^\star \mid \omega = \omega^{\texttt{rev}}\}$;
  \item
  ...
  \item
    $L = \{\alpha \sharp \beta \mid \alpha,\beta \in \{a,b\}^\star\ \&\ \alpha^{\texttt{rev}}\mbox{ е поддума на }\beta \}$.
  \item
...
\begin{problem}
  ...
  \item
    $L^\star$, където
    $L = \{\alpha\alpha^{\texttt{rev}} \mid \alpha \in \{a,b\}^\star\}$;
  \item
...
\begin{problem}
  ...
  \item 
    ...
    \marginpar{Да}
    \fi
    $\{\alpha \sharp \beta \mid \alpha,\beta \in \{a,b\}^\star\ \&\ |\alpha| = |\beta|\ \&\ \texttt{diff}(\alpha,\beta^{\texttt{rev}}) = 1\}$;
  \item
    \ifhints
    \marginpar{Да}
    \fi
    $\{\alpha \sharp \beta \mid \alpha, \beta \in \{a,b\}^\star\ \&\ |\alpha| = |\beta|\ \&\ \texttt{diff}(\alpha,\beta^{\texttt{rev}}) \geq 1\}$;
  \item
    \ifhints 
    \marginpar{Не}
    \fi
...
\begin{problem}
    Да разгледаме езика
    \[D_1 \df \{\alpha_1 \sharp \cdots \sharp \alpha_n \mid n \geq 2\ \&\ |\alpha_i| = |\alpha_{i+1}|\ \&\ \texttt{diff}(\alpha_i,\alpha^{\texttt{rev}}_{i+1}) = 1\}.\]
    \begin{itemize}
    \item 
...
\begin{hint}
  ...
  \[L_1 = \{\alpha_1 \sharp \alpha_2 \mid |\alpha_1| = |\alpha_{2}|\ \&\ \texttt{diff}(\alpha_1,\alpha^{\texttt{rev}}_{2}) = 1\}.\]
  Тогава
  \begin{align*}
