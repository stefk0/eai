...
\begin{problem}
  ...
  \[L_1 \triangle_n L_2 \df \{\alpha\beta \mid \alpha \in L_1\ \&\ \beta \in L_2\ \&\ |\alpha| = |\beta|\ \&\ \texttt{diff}(\alpha,\beta) = n\}.\]
  Докажете, че
  \begin{enumerate}[a)]
  \item 
    ако $L_1$ и $L_2$ са регулярни езици, то е възможно $L_1 \triangle_n L_2$ да не е регулярен;
  \item
    ако $L_1$ и $L_2$ са регулярни езици, то $L_1 \triangle_n L_2$ е безконтекстен;
  \item
    ако $L_1$ и $L_2$ са безконтекстни езици, то е възможно $L_1 \triangle_n L_2$ да не е безконтекстен.
  \end{enumerate}
\end{problem}
...
\begin{problem}
  ...
  \begin{enumerate}[a)]
  \item
    $S\rightarrow BA| CA|a, C\rightarrow BS|SA,A\rightarrow a, B\rightarrow b$,\\
    $\alpha=bbaaa$;
  \item
    $S\rightarrow AB|BC, A\rightarrow BA|a,B\rightarrow CC|b, C\rightarrow AB|a$,\\
    $\alpha=baaba$;
  \item
    $S\rightarrow AB, A\rightarrow AC|a|b,B\rightarrow CB|a, C\rightarrow a$,\\
    $\alpha=babaa$.
  \end{enumerate}
\end{problem}
...
\begin{hint}
  ...
  Това дали е вярно:
  \begin{align*}
    & S \to XSX\ |\ \sharp O\ |\ O\sharp\ |\ \sharp S\sharp\\
    & O \to OO\ |\ CS\ |\ SC\\
    & C \to XCX\ |\ \sharp\\
    & X \to a\ \|\ b.
  \end{align*}
