...
\begin{hint}
  ...
  \item 
    Нека $i = j$.
    Да разглеждаме думата $xy^{p^2+1}uv^{p^2+1}w$, която за да бъде в езика $L$ означава, че трябва $p+p^2i$ дели $p^2+p^2i$, т.е.
    $1 + pi$ трябва да дели $p+pi$.
    \begin{align*}
  ...
  \item
    ...
    За да бъде тази дума в езика $L$, трябва $p+p^2i$ да дели $p^2+p^2j$, т.е.
    $1 + pi$ трябва да дели $p+pj$, но 
...
Дефинираме функцията $\texttt{diff}:\Sigma^\star \times \Sigma^\star \to \Sigma^\star$ по следния начин:
\begin{align*}
  & \texttt{diff}(\alpha,\varepsilon) = 0\\
  & \texttt{diff}(\varepsilon, \beta) = 0\\
...
%   \begin{enumerate}[a)]
%   \item 
%     $L = \{\alpha\sharp\beta^{\rev} \mid \alpha,\beta \in (V \cup \Sigma)^\star\ \&\ \alpha \derive{1}_G \beta\}$;
%   \item 
%     $L = \{\alpha\sharp\beta^{\rev} \mid \alpha,\beta \in (V \cup \Sigma)^\star\ \&\ \alpha \derive{\star}_G \beta\}$;
%   \item
%     $L = \{\alpha\sharp\beta^{\rev} \mid \alpha,\beta \in (V \cup \Sigma)^\star\ \&\ \alpha \not\derive{1}_G \beta\}$;
%   \item
%     $L = \{\alpha\sharp\beta^{\rev} \mid \alpha,\beta \in (V \cup \Sigma)^\star\ \&\ \alpha \not\derive{\star}_G \beta\}$.
%   \end{enumerate}
%   Обосновете се!
% \end{problem}
