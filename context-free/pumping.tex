...
\begin{proof}
  ...
  \[2^{|V| + 1}\leq |\alpha| \leq 2^{\texttt{height}(T)}.\]
  Оттук следва, че
  \[|V| + 1 \leq \texttt{height}(T).\]
  Да фиксираме максимален път $\pi$ в $T$, т.е. дума $\pi \in T$ и $|\pi| = \texttt{height}(T)$.
  Ясно е, че
  \[|V| + 1 \leq |\{ X_\alpha \mid \alpha \prec \pi\}| = \texttt{height}(T),\]
  От принципа на Дирихле следва, че съществува $\beta \prec \pi$, за което
  \begin{equation}
    \label{eq:10}
    X_\beta \in \{X_\alpha \mid \beta \prec \alpha \prec \pi\}.
  \end{equation}
  Нека $\beta$ е най-дългата дума, за която е изпълнено свойството (\ref{eq:10}).
  Да означим с $\alpha$ една от думите, за която $\beta \prec \alpha \prec \pi$ и $X_\alpha = X_\beta$.
  От избора на $\beta$ следва, че
  \begin{equation}
    \label{eq:11}
    \texttt{height}(T_\beta) = |\{ X_\alpha \mid \beta \preceq \alpha \prec \pi\}| \leq |V| + 1.
  \end{equation}
  Нека $u = \texttt{yield}(P_\alpha)$. Тогава $yuv = \texttt{yield}(P_\beta)$.
  ...
  \item
    $\abs{yuv} \leq p$, защото
    \begin{align*}
      \abs{yuv} & \leq 2^{\texttt{height}(T_\beta)} & \comment\text{от \Cor{tree:upper-bound}}\\
                & \leq 2^{|V|+1} & \comment\text{от (\ref{eq:11})}\\
                & = p
    \end{align*}
  \item
    $xy^iuv^iw \in L$, защото можем да заменим поддървото $T_\alpha$ с поддървото $T_\beta$ и в новополученото дърво отново да заменим поддървото $T_\alpha$ с $T_\beta$
    и така нататък, краен брой пъти.
    В случая $i = 0$, заменяме поддървото $T_\beta$ с дървото $T_\alpha$.
  \end{enumerate}
\end{proof}
