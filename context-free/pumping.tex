...
\end{lemma}
\begin{proof}
  \mynote{$b$ е максималната разклоненост на всяко дърво на извод за дума от граматиката $G$.}
  ...
  Тогава от \Corollary{tree:upper-bound} следва, че ако $P = (T,\lambda)$ е дърво на извод за $\alpha$ в $G$, за което $X_\varepsilon = S$, то
  ...
  \item
    ...
    би имала дърво на извод $(\uppercut{P}{\beta}) \odot (\lowercut{P}{\gamma})$, защото:
    \begin{itemize}
    \item 
...
\begin{hint}
  ...
  \mynote{\writedown Защо са безконтекстни?}
  ...
  \item
    \mynote{Озн. $\ov{L} = \Sigma^\star \setminus L$}
