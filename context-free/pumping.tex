...
\end{proposition}
\begin{proof}
  Това твърдение е на практика реформулировка на \Corollary{tree:upper-bound}.
  \begin{itemize}
  \item
    Да разгледаме синтактично дърво $P$ за извода $A \yield{\leq\ell} \alpha$.
    Според \Corollary{tree:upper-bound} имаме, че $\abs{\alpha} \leq b^{\texttt{height}(P)} \leq b^{\ell}$.
  \item
...
\begin{proof}
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \draw [photon] (A) -- (D) -- (E) -- (F);
    \end{tikzpicture}
    \caption{\scriptsize{Най-долните две повторения на променлива по пътя $\pi$.}}
  ...
  \begin{subfigure}[t]{0.3\textwidth}
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \draw [photon] (A) -- (D) -- (E) -- (F);
    \end{tikzpicture}
    \caption{\scriptsize{Разбиваме дървото на три части.}}
    \end{subfigure}
    ~
    \begin{subfigure}[t]{0.3\textwidth}
      \centering
