...
\begin{problem}
  \mynote{Ясно е, че $\ell < b^{k}$.}
  Докажете, че ако $X \yield{k} \alpha$, то $X \stackrel{\ell}{\to}_G \alpha$, където
  \[\ell \leq \sum_{i < k}b^{i}.\]
\end{problem}
...
\mynote{Сравнете с \Proposition{grammar:derive-one-step}. Тук новото е, че имаме изискване $\delta \in \Sigma^\star$.}
\begin{proposition}\label{pr:grammar:left-derive-one-step}
  Нека $G$ е безконтекстна граматика. 
  Тогава за произволни думи $\alpha,\beta \in (V\cup\Sigma)^\star$ е изпълнено, че 
  $\alpha \lderive{1} \beta$ точно тогава, когато $\alpha = \delta A \rho$ и $\delta \in \Sigma^\star$, $A \to_G \gamma$ и $\beta = \delta\gamma\rho$.
\end{proposition}


% \begin{proposition}\label{pr:grammar:left-divide}
%   Нека $G$ е безконтекстна граматика и нека $X_1\cdots X_k \lderive{\ell} \beta$, където $X_i \in V \cup \Sigma$ и $\beta \in \Sigma^\star$.
%   Тогава съществуват думи $\beta_1,\dots,\beta_k$, за които $\beta = \beta_1\cdots\beta_k$ и за $i = 1,\dots, k$ е изпълнено, че
%   $X_i \lderive{\ell_i} \beta_i$ и $\ell = \sum^k_{i = 1}\ell_i$.
% \end{proposition}
...
\begin{hint}
  ...
  Индукция по $\ell$, където $A \yield{\ell}\alpha$.
\end{hint}
