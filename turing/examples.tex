...
\begin{example}
  ...
  \marginpar{Алгоритъмът има времева сложност $\mathcal{O}(n^2)$. Ако разгледаме машина на Тюринг с три ленти, то ще получим времева сложност $\mathcal{O}(n)$.}
  \begin{enumerate}[(1)]
  \item 
  ...
  \begin{figure}[H]
    \begin{center}
      \begin{tikzpicture}[->,>=stealth,thick,node distance=60pt]
        ...
        \node[state,accepting]  (6) [below of=1]{$q_5$};
        
        \begin{scope}[every node/.style={scale=.8}]
          \path
          (1) edge [loop above]    node [above] {$x;\goright$} (1)
          (1) edge [bend right=15] node [left]  {$\blank;\stay$} (6)
    ...
    \begin{center}
      \begin{tikzpicture}[->,>=stealth,thick,node distance=60pt]
        ...
        \begin{scope}[every node/.style={scale=.8}]
          ...
          (5) edge [loop right]    node [below right] {$\{a,b,x\};\goleft$} (5)
          (5) edge [in=65,out=115,above] node [above] {$\blank;\goright$} (1)
          (1) edge [dashed, bend right=15] node [left] {$\{b,c\};\stay$} (7)
...
\begin{example}
  \marginpar{Да напомним, че този език не е безконтекстен. В \cite[стр. 155]{hopcroft1} е дадено по-различно решение. Тук следваме \cite[стр. 173]{sipser3}. Там има малка грешка.}
  Да разгледаме езика
  ...
  \end{enumerate}
  \marginpar{Обърнете внимание, че този алгоритъм има времева сложност $\mathcal{O}(n^2)$. Ще видим в Пример~\ref{ex:multitape:omega:sharp:omega}, че ако разгледаме двулентова машина на Тюринг, то имаме алгоритъм със сложност $\mathcal{O}(n)$.}
