...
\begin{example}
  ...
  и за всяка двойка $(q,z)$, за която функцията на преходите не е дефинирана, да сочи към $\qreject = q_6$.
    ...
    \begin{center}
      \begin{tikzpicture}[->,>=stealth,thick,node distance=50pt]
        ...
        \begin{scope}[every node/.style={scale=.8}]
          ...
          (5) edge [dashed, loop below] node [below] {$\{a,b,c,x,\blank\};\stay$} (5)
          (6) edge [dashed, loop below] node [below] {$\{a,b,c,x,\blank\};\stay$} (6);
        \end{scope}
      \end{tikzpicture}
    \end{center}
  \end{figure}
