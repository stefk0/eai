...
\begin{example}
  ...
  Лесно се съобразява, че за $n > 0$, крайният недетерминиран автомат $\N$ на Фигура \ref{fig:nfa-exp} с $n+1$ на брой състояния разпознава езика
  ...
  за които
  \marginpar{Да напомним, че броят на всички думи с дължина $n$ над азбука с $k$ букви е $k^n$.}
...
\begin{proof}
  ...
  От конструкцията на $\N$ следва, че редицата от състояния $(q_i)^{l}_{i=0}$ описва приемащо изчисление на $\A_1$ върху $\alpha$.
