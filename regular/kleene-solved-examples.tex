  ...
  \begin{framed}
    \begin{figure}[H]
      \begin{center}
        ...
        \end{tikzpicture}
      \end{center}
      \caption{$\L(\A) = \L(\mathbf{1^\star 0 (0 + 1)^\star)}$}
      \label{fig:a1}
    \end{figure}
\end{framed}

Лесно се съобразява, че езикът на автомата от \Figure{a1} се описва с регулярния израз $\mathbf{1^\star 0 (0 + 1)^\star}$.
Следвайки означенията и конструкцията от доказателството на \Theorem{regular-kleene},
...
\begin{problem}
  Приложете конструкцията от Теоремата на Клини за да получите регулярен израз, който описва езика на автомата $\A$ изобразен на Фигура~\ref{fig:a2}.
  \begin{framed}
    \begin{figure}[H]
      \begin{center}
        \begin{tikzpicture}[->,>=stealth,thick,node distance=45pt]
...
{\scriptsize
  \begin{solution}
    От теоремата на Клини знаем, че $\L(\A) = \L(\mathbf{r}^3_{0,2})$, където:
  \begin{align*}
