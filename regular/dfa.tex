...
\begin{example}
  ...
  \begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
      ...
      \end{tikzpicture}
      \caption{$\{\omega \in \{a,b\}^\star \mid \card{\omega}{a} \geq 2\}$}
    \end{subfigure}
    \begin{subfigure}[b]{0.45\textwidth}
      \begin{tikzpicture}[->,>=stealth,thick,node distance=45pt]
        \tikzstyle{every state}=[circle,minimum size=20pt,auto]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
      \begin{tikzpicture}[->,>=stealth,thick,node distance=45pt]
        ...
        (2) edge [bend left=30] node   [below] {$a$}    (0);
      \end{tikzpicture}
      \caption{$\{\omega \in \{a,b\}^\star \mid \card{\omega}{a} \equiv 0\ (\bmod\ 3)\}$}
    \end{subfigure}
  \end{figure}    
\end{example}
...
\begin{proof}
  ...
  \item
    ...
    \begin{align*}
      \delta^\star( \pair{p,q}, \varepsilon) & =\pair{p,q} & \comment\text{от деф. на }\delta^\star\\
...
\begin{proof}
  ...
  \begin{framed}
    \begin{figure}[H]
      ...
      \caption{Автомат разпознаващ думите, които не съдържат две поредни срещания на $a$}
    \end{figure}
  \end{framed}
...
\end{framed}
  ...
  \marginpar{$\card{\omega}{a} \df $ броят на срещанията на буквата $a$ в думата $\omega$}
  \begin{enumerate}[a)]
  \item 
    $\delta^\star(q_{00}, \omega) = q_{00} \implies (\exists k,n\in\Nat)[\card{\omega}{a} = 2k\ \&\ \card{\omega}{b} = 2n]$;
  \item 
    $\delta^\star(q_{00}, \omega) = q_{01} \implies (\exists k,n\in\Nat)[\card{\omega}{a} = 2k\ \&\ \card{\omega}{b} = 2n+1]$;
  \item 
    $\delta^\star(q_{00}, \omega) = q_{10} \implies (\exists k,n\in\Nat)[\card{\omega}{a} = 2k+1\ \&\ \card{\omega}{b} = 2n]$;
  \item 
    $\delta^\star(q_{00}, \omega) = q_{11} \implies (\exists k,n\in\Nat)[\card{\omega}{a} = 2k+1\ \&\ \card{\omega}{b} = 2n+1]$;
  \end{enumerate}
