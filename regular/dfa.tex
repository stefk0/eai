...
\begin{dfn}
  ...
  \item
    $\Sigma$ е азбука;
  \item
    \marginpar{Възможно е за някои двойки $(q,a)$ функцията $\delta$ да не е дефинирана.}
  ...
  \end{enumerate}
\end{dfn}
\marginpar{Възможно е да се позволят и $\varepsilon$-преходи, но това е излишно}
...
\begin{hint}
  ...
  \begin{itemize}
  \item
    $|\alpha| = 0$, т.е. $\alpha = \varepsilon$. Тогава за произволна дума $\beta$ имаме:
    \[\delta^\star(q, \varepsilon\beta) = \delta^\star( \underbrace{\delta^\star(q, \varepsilon)}_{q}, \beta).\]
  \item
    Да приемем, че твърдението е изпълнено за думи $\alpha$ с дължина $n$.
  \item
    Нека $|\alpha| = n+1$, т.е. $\alpha = a\cdot \gamma$, където $|\gamma| = n$. Тогава за произволна дума $\beta$ имаме:
    \begin{align*}
      \delta^\star(q, \alpha\beta) & = \delta^\star(q, a\gamma\beta)\\
                                   & = \delta^\star(\delta(q,a), \gamma\beta) & \comment{\text{от деф. на }\delta^\star}\\
                                   & = \delta^\star(\delta^\star(\delta(q,a), \gamma), \beta) & \comment{\text{от И.П. за }\gamma}\\
                                   & =\delta^\star(\delta^\star(q,a\gamma), \beta) & \comment{\text{от деф. на }\delta^\star}\\
...
Получаваме, че 
\[\L(\A) = \{\alpha\in\Sigma^\star \mid (\exists q \in F)[(\qstart,\alpha) \vdash^\star_\A(p,\varepsilon)]\}.\]
...
\begin{proof}
 ...
 то $\delta^\star(q_0,\alpha) \in \{q_0,q_1\}$ и от $(1)$ и $(2)$ следва, че и в двата случа
...
\begin{problem}
  Докажете, че следният език е автоматен:
...
\end{prop}
\begin{hint}
  \marginpar{Защо изискваме $\A'$ и $\A''$ да са тотални?}
  ...
  \item
    ...
    \marginpar{\writedown Проверете, че $\L(\A) = \L(\A')\cup \L(\A'')$}
