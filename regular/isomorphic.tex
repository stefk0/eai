  ...
  \item
    ...
    \begin{align*}
      f(\delta^\star_{\A'}(q,\beta c)) & = f(\delta_{\A'}(\delta^\star_{\A'}(q,\beta), c)) & \comment{\text{от деф. на }\delta^\star_{\A'}}\\
                                       ...
                                       & = \delta_{\A''}( \delta^\star_{\A''}(f(q),\beta), c) & \comment{\text{от И.П.}}\\
...
\end{framed}
\begin{hint}
  \marginpar{Лесно можем да съобразим, че в общия случай нямаме обратната посока на тази теорема.}
...
\begin{framed}
  \begin{thm}
    ...
    Нека \[\A = \FA\] е краен детерминиран автомат, за който $\L(\A) = L$ и $\abs{Q} = \abs{\Sigma^\star/_{\approx_L}}$.
