...
\begin{example}
  ...
  \mynote{Да напомним, че в \Example{brzozowski-solved-examples-2} вече видяхме автоматът на Бжозовски за $L$. Тук пишем $L_a$ и $L_{ab}$ вместо $a^{-1}(L)$ и $(ab)^{-1}(L)$ за да спестим място.}
  \begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
      \begin{tikzpicture}[framed,->,>=stealth,thick,node distance=55pt,initial text=начало,inner sep=1pt]
        ...
        (N) edge [bend right=15] node [above] {$a$} (M)
