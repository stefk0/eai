...
\begin{proof}
  ...
  Тогава от \Proposition{well-defined-pullback} следва, че $f(q_\alpha) = \hat{M}$, където $M = \alpha^{-1}(L)$, защото $\alpha^{-1}(L) = \L_\A(q_\alpha)$.
  
  Освен това, $f$ е сюрективна, защото, според дефиницията на автомат на Бжозовски, за произволно състояние $\hat{M} \in Q^\B$ има дума $\alpha$,
...
\begin{proof}
  ...
  \item
    ...
    Тогава имаме, че $f(p) = f(q_{\alpha b}) = \hat{N}$, защото $(\alpha b)^{-1}(L) = \L_\A(q_{\alpha b})$ отново според \Proposition{well-defined-pullback}.
