...
\begin{proof}
  ...
  \mynote{Изчислението на автомата $\A$ върху думата $\alpha$ едновременно симулира изчислението на $\A'$ и $\A''$ върху $\alpha$.}
  \begin{itemize}
  \item
    $Q \df Q'\times Q''$;
  \item
...
\end{framed}
\begin{hint}
  \mynote{\writedown Проверете, че $\Sigma^\star\setminus L = \L(\A')$. Съобразете, че тук е важно, че $\delta$ е тотална функция на преходите.}
