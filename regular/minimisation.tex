...
\begin{framed}
  \begin{prop}
    ...
    Нека $\A = \FA$ е краен детерминиран автомат {\em без недостижими от} $\qstart$ състояния.
    Тогава
    \[\abs{Q/_{\equiv_\A}} = \abs{\Sigma^\star/_{\approx_{\L(\A)}}}.\]  
  \end{prop}  
\end{framed}
\begin{proof}
  Нека $L = \L(\A)$ и да разгледаме $f:\Sigma^\star/_{\approx_L} \to Q/_{\equiv_\A}$, където
...
При даден език $L$ и краен детерминиран автомат $\A = \FA$, който го разпознава, целта ни е да построим нов краен детерминиран автомат $\M$,
