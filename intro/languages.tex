...
\item
  \mynote{Обърнете внимание, че $\emptyset\cdot A = A\cdot\emptyset = \emptyset$}
  \mynote{Също така, $\{\varepsilon\}\cdot A = A\cdot\{\varepsilon\} = A$}
...
\mynote{Тази операция ще бъде важна за нас по-нататък}
\[\alpha^{-1}(L) = \{\omega \in \Sigma^\star \mid \alpha\omega \in L\}.\]
Освен това, да означим 
\mynote{$\alpha(L) = \{\alpha\} \cdot L$}
