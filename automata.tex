...
\begin{proof}
  ...
  \item
    ...
    \[\mathbf{r^{k-1}_{i,j} + r^{k-1}_{i,k}\cdot (r^{k-1}_{k,k})^\star\cdot r^{k-1}_{k,j}}.\]
  \end{enumerate}
...
\begin{example}
 ...
 Следвайки конструкцията от доказателството на Теорема \ref{th:regular-kleene},
...
\begin{lemma}
  ...
  ако $\L(\N) = L$, то съществува $\N_1$, за който $\L(\N_1) = L^\star$.
\end{lemma}
\begin{proof}
...
\begin{example}
  ...
  Лесно се вижда, че $\L(\N_3) = \{b\} \cup \{(abb)^nab\mid n\in\Nat\}$.
  Формално погледнато, след като построим автомат за езика $\L(\N_3)^+$, трябва да приложим
  конструкцията за обединение на автомата за езика $\L(\N_3)^+$ с автомата за езика $\{\varepsilon\}$.
...
\begin{hint}
  ...
  \[s\stackrel{a_1}{\rightarrow} q_1 \stackrel{a_2}{\rightarrow} \dots \stackrel{a_p}{\rightarrow} q_p.\]
