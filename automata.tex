...
\begin{proof}
  ...
  \begin{itemize}
  \item 
    \marginpar{Трябва да проверим, че $(\forall b,b')(f(b) \neq f(b')\ \rightarrow b \neq b')$}
    $f$ е функция, защото за $[\alpha]_\M = [\beta]_\M$, 
    $\alpha \sim_\M \beta$ и от \Th{rel-finer} веднага следва, че $[\alpha]_L = [\beta]_L$.
  \item
    \marginpar{$(\forall a)(\exists b)(f(b) = a)$}
...
%  \marginpar{стр. 96 от \cite{papadimitriou}}
  ...
  \begin{itemize}
  \item
    \marginpar{Трябва да проверим, че $Q$ е крайно.}
  ...
  \item
    \marginpar{Трябва да проверим, че $\delta$ е функция.}
...
\begin{proof}
  ...
  \item
    ...
    Понеже $w_q \sim_\M \alpha$, от \Th{rel-finer} следва директно, че $[w_q]_L = [\alpha]_L$.
    Това означава, че $f$ е определена коректно, т.е. $f$ е {\bf функция}.
  % \item
%    \marginpar{$(\forall p,q)(f(p) \neq f(q)\ \rightarrow\ p \neq q)$}
    % Ще проверим, че $f$ е определена коректно, т.е. $f$ е {\bf функция}.
