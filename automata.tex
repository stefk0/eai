...
често е удобно да разглеждаме (частичната) функция $\delta^\star:Q\times\Sigma^\star \to Q$, като е дефинирана по следния начин:
...
\end{lemma}
\begin{proof}
  Нека е даден автомата $\N = \NFA$, за който е изпълнено, че
  $L(\N) = L(r)$.
...
\begin{proof}
  ...
  \item
    \marginpar{Няма общо правило, което да ни казва как избираме думата $\alpha$. Възможно е да пробваме с няколко думи $\alpha$, докато намерим такава, която върши работа}
...
%  \marginpar{стр. 96 от \cite{papadimitriou}}
  ...
  За целта, първо ще докажем две помощни твърдения.
