...
\item
  ...
  $\delta_\A(q_M,a) = q_N$.
\item
  $b^{-1}M = \{a,b\}^\star aL = M$. Следователно, $\delta_\A(q_M,b) = q_M$.
\item
  $a^{-1}N = a^{-1}L \cup L \cup aL \cup \{a,b\}^+aL = M \cup L \cup \{a,b\}^\star aL = M \cup L \cup M = N$. Следователно, $\delta_\A(q_N,a) = q_N$.
\item
...
Да разгледаме изображението $f$ с дефиниционна област $\Sigma^\star/_{\sim_L}$, дефинирано като:
\[f([\alpha]_L) = q_M, \text{ където }M = \alpha^{-1}L.\]

% Трябва да построим биекция $f:Q^\M \to Q^\A$, така че
