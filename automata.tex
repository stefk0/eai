...
\item
  Съобразете, че всяко състояние на $\M$, което е достижимо от началното състояние, определя клас на еквивалентност относно 
  релацията $\sim_\M$.  
\item
  $\sim_\M$ е дясно-инвариантна, т.е. за всеки две думи $\alpha$ и $\beta$,
  \[\alpha \sim_\M \beta \implies (\forall \gamma\in\Sigma^\star)[\alpha\gamma \sim_\M \beta\gamma].\]
\end{itemize}

\begin{thm}
  \label{th:rel-finer}
  Нека е даден ДКА $\M = \FA$ и $\L(\M)$ е езика, разпознаван от $\M$. Тогава е изпълнено
  \[(\forall \alpha,\beta \in \Sigma^\star)[\alpha\sim_\M\beta \implies \alpha\approx_{\L(\M)}\beta].\]
\end{thm}
\begin{proof}
